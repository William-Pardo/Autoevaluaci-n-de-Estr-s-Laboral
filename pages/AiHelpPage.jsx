...
const AiHelpPage = () => {
  ...
  useEffect(() => {
    if (submission && messages.length === 0) {
      ...
      const newMessages = [{ role: 'user', content: initialUserMessage }];
  ...
  const handleSendMessage = async (messageText) => {
    ...
    // Add user message to UI immediately if it's a new message
    if (messages.length === 0 || messageText !== messages[messages.length-1]?.content) {
  ...
  if (!submission) {
    // If user lands here directly without submission data, redirect them.
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg flex flex-col h-[calc(100vh-150px)]">
      ...
      <div className="p-4 border-t border-gray-200 bg-white">
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(userInput); }} className="flex items-center gap-4">
          <input
            type="text"
            value={userInput}
